...
public class DeptRepositoryImpl extends BaseRepositoryImpl<TDept, Integer> implements DeptRepository {
	...
	public List<DeptBean> getListDept(DeptForm deptForm) {
	    ...
	    sql.append("SELECT dept.id,supplier_name,money_own,money_paided,convert(varchar,date_start,23) date_start,convert(varchar,date_end,23) date_end,limit,over_limit,dept.id a ");
...
//	    }
	...
	public List<DeptBean> getDept(String id) {
		StringBuilder sql = new StringBuilder();
	    sql.append("SELECT id,fk_supplier_id,money_own,money_paided,convert(varchar,date_start,23) date_start,convert(varchar,date_end,23),limit,over_limit,term,note ");
	    sql.append("FROM DEPT dept ");
	...
	public void insertDept(TDept tDept) {
		StringBuilder sql = new StringBuilder();
	    sql.append("INSERT INTO DEPT(fk_supplier_id,money_own,money_paided,date_start,date_end,limit,over_limit,term,note,status) ");
	    sql.append("VALUES(?,?,?,?,?,?,?,?,?,'0') ");
	    Query query = entityManager.createNativeQuery(
	    		sql.toString());
	    query
	    .setParameter(1, tDept.getFk_supplier_id())
	    .setParameter(2, tDept.getMoney_own())
	    .setParameter(3, tDept.getMoney_paided())
	    .setParameter(4, tDept.getDate_start())
	    .setParameter(5, tDept.getDate_end())
	    .setParameter(6, tDept.getLimit())
	    .setParameter(7, tDept.getOver_limit())
	    .setParameter(8, tDept.getTerm())
	    .setParameter(9, tDept.getNote())
	    .executeUpdate();
	}
	...
	public void updateDept(TDept tDept) {
		StringBuilder sql = new StringBuilder();
	    sql.append("UPDATE DEPT SET fk_supplier_id = ?1,money_own = ?2,money_paided = ?3,date_start = ?4,date_end = ?5,limit = ?6,over_limit = ?7, term = ?8, note = ?9 ");
