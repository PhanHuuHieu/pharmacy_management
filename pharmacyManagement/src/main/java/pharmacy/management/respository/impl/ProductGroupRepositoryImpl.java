...
@Repository
public class ProductGroupRepositoryImpl extends BaseRepositoryImpl<TProductGroup, Integer> implements ProductGroupRepository {
	...
	public List<TProductGroup> getListProductGroup() {
	    ...
	    Query query = entityManager.createNativeQuery(
	    		sql.toString(),TProductGroup.class);	
	...
	public List<TProduct> getListProductRelated(String id) {
	    ...
	    sql.append("SELECT DISTINCT pro.id,pro.name,pro.note, pro.price_sell,pro.picture,pro.description,pro.weight,pro.color ");
	    sql.append("FROM PRODUCT pro ");
	    sql.append("WHERE pro.fk_product_group_id = (SELECT fk_product_group_id FROM PRODUCT WHERE id = ?1)");
