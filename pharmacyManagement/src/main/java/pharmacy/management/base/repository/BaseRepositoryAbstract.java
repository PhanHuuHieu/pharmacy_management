 ...
 * @param <ID>
 */
public abstract class BaseRepositoryAbstract<T, ID extends Serializable> implements BaseRepository<T, ID> {
	...
	public List<T> getAll() {
		Query query = (Query) this.entityManager.createNativeQuery("from " + persistentClass.getName());
		return query.list();
	...
	public T getById(ID id) {
		Query query = (Query) this.entityManager
				.createNativeQuery("from " + persistentClass.getName() + " where " + fieldId.getName() + " = :idValue");
