...
public class ProductRepositoryImpl extends BaseRepositoryImpl<TProduct, Integer> implements ProductRepository {
	...
	public void insertProduct(TProduct tproduct) {
		...
		sql.append(
				"INSERT INTO PRODUCT(status,name,weight,color,date_manufature,date_expirate,price_sell,price_orginal,fk_supplier_id,fk_product_group_id,fk_unit_id,note,picture,created_date,picture1,picture2) ");
		sql.append("VALUES(0,?,?,?,?,?,?,?,?,?,?,?,?,GETDATE(),?,?) ");
		Query query = entityManager.createNativeQuery(sql.toString());
		query.setParameter(1, tproduct.getName()).setParameter(2, tproduct.getWeight())
				.setParameter(3, tproduct.getColor()).setParameter(4, tproduct.getDate_manufature())
				.setParameter(5, tproduct.getDate_expirate()).setParameter(6, tproduct.getPrice_sell())
				.setParameter(7, tproduct.getPrice_orginal()).setParameter(8, tproduct.getFk_supplier_id())
				.setParameter(9, tproduct.getFk_product_group_id()).setParameter(10, tproduct.getFk_unit_id())
				.setParameter(11, tproduct.getNote()).setParameter(12, tproduct.getPicture())
				.setParameter(13, tproduct.getPicture1()).setParameter(14, tproduct.getPicture2()).executeUpdate();
	}
	...
	public List<TProduct> getProduct(int idProduct) {
		StringBuilder sql = new StringBuilder();
		sql.append(
				"SELECT  DISTINCT pro.name, price_orginal, price_sell, weight, color, convert(varchar,date_manufature,23) date_manufature, convert(varchar,date_expirate, 23), fk_product_group_id, fk_supplier_id, fk_unit_id, pro.note, pro.id,pro.picture,description,pro.picture1,pro.picture2 ");
	...
	public void updateProduct(TProduct tproduct) {
		...
		sql.append(
				"price_sell = ?6,price_orginal = ?7,fk_supplier_id = ?8,fk_product_group_id = ?9,fk_unit_id = ?10,note = ?11");
		if (!tproduct.getPicture().equals("/images/")) {
			sql.append(",picture = '" + tproduct.getPicture() + "'");
		}
		if (!tproduct.getPicture1().equals("/images/")) {
			sql.append(",picture1 = '" + tproduct.getPicture1() + "'");
		}
		if (!tproduct.getPicture2().equals("/images/")) {
			sql.append(",picture2 = '" + tproduct.getPicture2() + "'");
		}
		...
		query.setParameter(1, tproduct.getName()).setParameter(2, tproduct.getWeight())
				...
				.setParameter(7, tproduct.getPrice_orginal()).setParameter(8, tproduct.getFk_supplier_id())
				.setParameter(9, tproduct.getFk_product_group_id()).setParameter(10, tproduct.getFk_unit_id())
				.setParameter(11, tproduct.getNote()).setParameter(12, tproduct.getId()).executeUpdate();
	}
	...
	public List<ProductBean> getProductSearch(ProductForm productForm) {
		StringBuilder sql = new StringBuilder();
		sql.append(
				"SELECT  DISTINCT pro.id, pro.name, price_orginal, price_sell, weight, color, product_group_name, pro.picture, pro.id c ");
		sql.append("FROM         PRODUCT pro ");
		...
		if (!productForm.getName().equals("")) {
			// sql.append("AND CONTAINS(pro.name,'\""+productForm.getName()+"*\"') ");
			sql.append("AND name LIKE N'%" + productForm.getName() + "%' ");
		}
		if (productForm.getPrice_orginal() != 0) {
			sql.append("AND price_orginal >= " + productForm.getPrice_orginal() + " ");
		}
		if (!productForm.getPrice_sell().equals("0")) {
	...
	public void THUXINHDEP(String id, String label) {
		...
		// PRODUCT(name,fk_supplier_id,fk_product_type_id,fk_product_group_id,fk_unit_id)
		// VALUES(?,1,1,1,1) ");
		Query query = entityManager.createNativeQuery(sql.toString());
		query.setParameter(1, label).setParameter(2, id).executeUpdate();
	}
