...
public class StockRepositoryImpl extends BaseRepositoryImpl<TStock, Integer> implements StockRepository {
	...
	public List<StockBean> getStock(String id) {
		StringBuilder sql = new StringBuilder();
		sql.append(
				"SELECT type_form,convert(varchar,date,23),stock_name,type,fk_product_id,fk_employee_id,status,note,id ");
		sql.append("FROM STOCK stock ");
	...
	public void insertStock(TStock stockBean) {
		StringBuilder sql = new StringBuilder();
		sql.append(
				"INSERT INTO STOCK(type_form,date,stock_name,type,status,fk_employee_id,fk_product_id,note,is_delete) ");
		sql.append("VALUES(?,?,?,?,?,?,?,?,'0') ");
		Query query = entityManager.createNativeQuery(sql.toString());
		query.setParameter(1, stockBean.getType_form()).setParameter(2, stockBean.getDate())
				.setParameter(3, stockBean.getStock_name()).setParameter(4, stockBean.getType())
				.setParameter(5, stockBean.getStatus()).setParameter(6, stockBean.getFk_employee_id())
				.setParameter(7, stockBean.getFk_product_id()).setParameter(8, stockBean.getNote()).executeUpdate();
	}
	...
	public void updateStock(TStock stockBean) {
		StringBuilder sql = new StringBuilder();
		sql.append(
				"UPDATE STOCK SET type_form = ?1,date = ?2,stock_name = ?3,type = ?4,status = ?5,fk_employee_id = ?6,fk_product_id = ?7, note = ?8 ");
		sql.append("WHERE id = ?9");
		Query query = entityManager.createNativeQuery(sql.toString());
		query.setParameter(1, stockBean.getType_form()).setParameter(2, stockBean.getDate())
				.setParameter(3, stockBean.getStock_name()).setParameter(4, stockBean.getType())
				.setParameter(5, stockBean.getStatus()).setParameter(6, stockBean.getFk_employee_id())
				.setParameter(7, stockBean.getFk_product_id()).setParameter(8, stockBean.getNote())
				.setParameter(9, stockBean.getId()).executeUpdate();
	}
	...
	public List<StockBean> getListExport(StockExportForm stockExportForm) {
		...
		for (Object[] obj : listReportOrderProductObject) {
			StockBean orderBean = new StockBean(String.valueOf(obj[0]), String.valueOf(obj[1]), String.valueOf(obj[2]),
